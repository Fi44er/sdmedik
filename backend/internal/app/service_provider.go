...
func (s *serviceProvider) initPageProvider() error {
	s.pageProvider = *provider.NewPageProvider(s.logger, s.db, s.validator)
	return nil
}
...
func (s *serviceProvider) initAuthProvider() error {
	s.authProvider = *provider.NewAuthProvider(s.logger, s.validator, s.config, s.cache, s.eventBus, s.userProvider.UserService(), s.basketProvider.BasketService())
	return nil
}
...
func (s *serviceProvider) initOrderProvider() error {
	s.orderProvider = *provider.NewOrderProvider(
		s.logger,
		s.validator,
		s.db,
		s.config,
