...
import (
	...
	database "github.com/Fi44er/sdmedik/backend/database/postgres"
	redis_connect "github.com/Fi44er/sdmedik/backend/database/redis"
	events "github.com/Fi44er/sdmedik/backend/shared/eventbus"
	"github.com/Fi44er/sdmedik/backend/shared/logger"
	"github.com/Fi44er/sdmedik/backend/shared/middleware"
...
func (app *App) initDb() error {
	if app.db == nil {
		db, err := database.ConnectDb(app.config.PostgresUrl, app.logger)
		if err != nil {
			return err
		}
		app.db = db
		...
		if err := database.Migrate(db, app.migrate, app.logger); err != nil {
...
func (app *App) initEvenBus() error {
	if app.eventBus == nil {
...
func (app *App) initRouter() error {
	...
	deserializeUser := middleware.DeserializeUser(app.redis, app.db, app.config)

	app.moduleProvider.userModule.RegisterRoutes(api)
	app.moduleProvider.authModule.RegisterRoutes(api, deserializeUser)
	app.moduleProvider.productModule.RegisterRoutes(api)
	app.moduleProvider.categoryModule.RegisterRoutes(api)
